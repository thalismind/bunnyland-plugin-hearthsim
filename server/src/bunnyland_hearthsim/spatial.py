"""Spatial helpers shared across the cooking mechanics.

``container_of`` only returns an entity's *direct* ``Contains`` parent. These helpers answer
the two questions the cooking logic actually asks:
...
- ``room_of(entity)`` — which room is this entity ultimately in, whether loose on the floor
  or nested in a character's inventory?
"""
